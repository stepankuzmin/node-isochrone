...
test('deintersected isochrone', (t) => {
  ...
  points.forEach(point => isochrone(point, Object.assign({}, options, { deintersect: true }))
    .then((geojson) => {
      ...
      const isochrones = options.intervals.reduce(
        (acc, interval) => Object.assign({}, acc, {
...
test('isochrone', (t) => {
  t.plan(points.length * (1 + testPairs.length));
  points.forEach(point => isochrone(point, Object.assign({}, options, { deintersect: false }))
    .then((geojson) => {
      const errors = geojsonhint.hint(geojson);
      if (errors.length > 0) {
        errors.forEach(error => t.comment(error.message));
